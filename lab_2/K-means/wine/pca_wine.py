...
for i in range(len(cluster_colors)):
    plt.scatter(X_2D[df.index[wine.target == i], 0], X_2D[df.index[wine.target == i], 1], label=f'Class {i}', c=cluster_colors[i], alpha=0.7)
...
for i in range(len(cluster_colors)):
    ax.scatter3D(X_3D[df.index[wine.target == i], 0], X_3D[df.index[wine.target == i], 1], X_3D[df.index[wine.target == i], 2], label=f'Class {i}', c=cluster_colors[i], alpha=0.7)

ax.set_title('PCA Visualization of Wine Dataset (3D)')
