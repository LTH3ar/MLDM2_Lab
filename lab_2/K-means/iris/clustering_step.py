...
for i in range(1, 5):
    ...
    # Scatter plot side by side for the next kmens
    plt.scatter(centers[:, 0], centers[:, 1], marker='x', s=100, c='red', label='Centroids')
    plt.xlabel('PCA1')
    plt.ylabel('PCA2')
    plt.title(f'Iris Cluster with Kmeans = {i}')
    plt.legend()
    plt.show()